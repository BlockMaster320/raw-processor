...
public:
    ...
    void initializeGL();
    void cleanupGL();
    void processImage(std::shared_ptr<Image> image, bool gammaCorrect = false);
    bool processForExport(std::shared_ptr<Image> image, std::vector<unsigned char>& outBuffer, int& outWidth, int& outHeight);
...
private:
    ...
    QOpenGLShaderProgram gammaProgram;
